@include('admin.nyquist-experiment.components.step1')
<div class="row">
...
<div class="row mb-5">
    ...
    <div class="col-12 offset-md-1 col-md-5">
        ...
        <div class="form-group row align-items-center" :class="{'has-danger': errors.has('layout_id'), 'has-success': fields.layout_id && fields.layout_id.valid }">
            <label for="layout_id" class="col-12 col-form-label">{{ trans('admin.experiment.columns.layout_id') }}</label>
                <div class="col-12">
                    <multiselect v-model="form.layout" :options="{{ $layouts->toJson() }}" placeholder="{{ trans('admin.experiment.columns.layout_id') }}" label="name" track-by="id" :multiple="false"></multiselect>
    ...
    <div class="col-12 col-md-12 text-center d-flex justify-content-center" v-if="form.layout && form.ajax_url">
        <button type="submit" @click.prevent="getResponsesFromServer" class="btn btn-primary d-flex text-uppercase">{{ __('admin.experiment.steps.go_to_third') }}</button>
    </div>
</div>
@include('admin.nyquist-experiment.components.step3')
